...
- 加载未打包 Chrome 扩展（`--load-extension` + `--disable-extensions-except`）**必须**使用 bundled Chrome for Testing（`channel=None`，走 Patchright 默认），**不能**用系统 Google Chrome（`channel='chrome'`）。Chrome 137+ 反恶意策略在 branded Chrome 启动日志里明确打印 `WARNING: --load-extension is not allowed in Google Chrome, ignoring.`，命令行开关被直接吞掉。代价是失去 stealth-browser 的"11 项指纹差距"反检测优势，但扩展测试不需要反检测。`engine.launch(extensions=...)` 已自动切换；调用方不要再强塞 `channel='chrome'`
