...
- 描述: 集成 human-cursor 实现鼠标轨迹模拟（贝塞尔曲线 + Fitts 定律 + overshoot + 微抖动）。打字模拟：字符间不均匀延迟、3%~12% 偶发错字 + 退格修正、空格后短暂停顿。滚动模拟：惯性减速、随机停顿、偶尔回滚。点击前自动 hover 50~300ms。所有行为参数随机化，不产生可识别的模式。
...
- 描述: 检测页面中的滑块 CAPTCHA 元素，截图后用 OpenCV 图像分析确定拼图缺口位置，通过 F3 的人类行为模拟拖动滑块到正确位置。自动尝试最多 2 次，失败后截图并报告用户。支持拼图滑块和旋转滑块两种类型。
