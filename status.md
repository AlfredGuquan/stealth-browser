...
- 加载 unpacked Chrome extension（`--extension` flag + `STEALTH_BROWSER_EXTENSIONS` env）：engine.launch 接 extensions 参数，自动切 `launch_persistent_context` + bundled Chrome for Testing（branded Chrome 137+ 拒绝 --load-extension），强制 headed，单独 profile 目录 `~/.stealth-browser/ext-profile`。CLI `--extension <path>`（可重复）+ env fallback（`os.pathsep` 分隔，explicit flag 覆盖 env）。fixture `tests/fixtures/minimal_extension/` + e2e `test_extension_e2e.py`（PASS 17.68s）。269 单测 PASS。CLAUDE.md 沉淀 bundled Chrome 约束。Personal-website Web Clipper 完整 4 层端到端验证 PASS（`tests/spike_extension_full_e2e.py`）：L1 manifest 加载 / L2 service worker 注册 / L3a chrome.runtime 在 SW 可用 / L3b chrome.runtime.sendMessage 跨 page→content→background 路径回话正常（reply keys=author,content,...）/ L3c trusted-click 桥接（CustomEvent 跨 main↔isolated world 会丢 user activation，必须 isolated world 直接监听 click）/ L4 sidepanel 经 `chrome-extension://EXT_ID/sidepanel.html` 直接 navigate 打开（dev-browser-setup.sh canonical flow）+ ws 连接 `ws://localhost:3142/ws` 由 sidepanel.js 自动建立。**已知限制**：scripted 路径调 `chrome.sidePanel.open()` 报 `may only be called in response to a user gesture`——绕过法走直接 navigate 到 sidepanel.html；Chrome 政策本身限制，非工具缺陷。[2026-04-26]
